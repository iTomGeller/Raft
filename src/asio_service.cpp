...
{
public:
    fs_based_logger(const std::string& log_file, raft::asio_service::log_level level)
...
private:
    raft::asio_service::log_level level_;
    std::ofstream fs_;
...
//在implementation提供io_svc 与 logger 
...
//enable_shared_from_this 可以解决多个shared_ptr指向this 而引用计数失效的情况
...
private:
    ...
    void read_complete()
    {
        ...
        {
            header_->pos(0);
            //整理出数据 header只需要一份 但log_data可以是多份
...
void raft::impls::fs_based_logger::flush()
{
...
raft::impls::fs_based_logger::~fs_based_logger()
{
    flush();
    fs_.flush();
    fs_.close();
...
void raft::impls::fs_based_logger::write_log(const std::string& level, const std::string& log_line)
{
